...
长任务最大的坑不是"不会做"，是**会做但重启后忘了已经做过/已经否决过**。会话压缩、换 agent、`/loop --resume`、第二天重开——每一次都是状态交接，交接丢了就重做或踩已否决的坑。两条纪律：

- **否决要落盘，且带理由**。被取消/拒绝的决策别只留在对话里，写进一个 `decisions.md`（或 CLAUDE.md 里的"已放弃的方案"小节），记**动作 + 日期 + 理由**。带理由的 veto 能跨会话存活、能被新证据推翻；没理由的 veto 一换环境就失效。Agent 不只忘事实，更忘否决——**负面知识和正面摘要一样重要**。
- **恢复不重放副作用**。`/loop --resume`、SessionStart 恢复、子 agent 重跑时，已执行过的写操作（开 PR、发请求、改文件）要能识别"已经做过"而不重打第二遍。落地产物用幂等键或"先查再写"，进度文件（`PROGRESS.md`）记录已完成步骤。真正的可恢复是 **"resume 不破坏现实"**，不是"我们存了点状态"。

这两条和 §9 的可剥离是两面：可剥离管"该删什么"，状态可恢复管"该留什么、怎么留才接得住下一次启动"。
